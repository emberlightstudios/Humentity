...
impl CharacterRagdoll {
    ...
    pub(crate) fn spawn_ragdoll(
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
                rig_entity, config, RagdollBone::Head, commands);

        let (collider, transform) = self.get_midsection_collider(helpers,RagdollBone::Chest);
        let name = self.get_torso_bone_name(rig_type);
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::Chest, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::Pelvis, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::UpperLeftArm, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::UpperRightArm, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::LowerLeftArm, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::LowerRightArm, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::UpperLeftLeg, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::UpperRightLeg, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::LowerLeftLeg, commands);
        ...
        self.insert_collider(collider, transform * rig_transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::LowerRightLeg, commands);
        ...
        self.insert_collider(collider, transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::LeftHand, commands);
        ...
        let transform = Transform::from(global_transforms.get(bone).unwrap().clone()) * transform;
        self.insert_collider(collider, transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::RightHand, commands);
        ...
        let transform = Transform::from(global_transforms.get(bone).unwrap().clone()) * transform;
        self.insert_collider(collider, transform, name, bone_entities, global_transforms,
            rig_entity, config, RagdollBone::LeftFoot, commands);
    ...
    fn insert_collider(
        ...
        let &parent = if let Some(parent) = bone_entities.get(parent_name)
            { parent } else { &rig_entity };
        let parent_transform = global_transforms.get(parent).unwrap();
        let transform = Transform::from_matrix(
             parent_transform.to_matrix().inverse() * transform.to_matrix()
        );
        let collider = commands.spawn((transform, collider)).id();
        let rb = commands.spawn((
            Transform::IDENTITY,
            //RigidBody::Dynamic,
            //RigidBodyDisabled,
        )).id();
    ...
    fn get_midsection_collider(&self, helpers: &Vec<Vec3>, joint: RagdollBone) -> (Collider, Transform) {
    ...
    fn get_limb_collider(&self, helpers: &Vec<Vec3>, joint: RagdollBone) -> (Collider, Transform) {
        ...
        for (i, &mhv) in ref_verts.iter().enumerate() {
            verts[i] = helpers[mhv];
            if   matches!(joint, RagdollBone::LowerRightArm) || matches!(joint, RagdollBone::UpperRightArm)
              || matches!(joint, RagdollBone::LowerRightLeg) || matches!(joint, RagdollBone::UpperRightLeg)
            {
                verts[i].x = -verts[i].x;
            }
        }
        ...
        (
            Collider::capsule_endpoints(r, p1, p2),
            Transform::IDENTITY
        )
    }

    fn get_extremity_collider(&self, helpers: &Vec<Vec3>, joint: RagdollBone) -> (Collider, Transform) {
        let ref_verts = match joint {
